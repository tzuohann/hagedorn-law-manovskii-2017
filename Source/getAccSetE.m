function [jxAccSetE,Cutoff] = getAccSetE(RD,C,SimO,M,Sim,P,NumjY,JobGroup,iDropped)
  ...
  if P.Pphi == 0
    jxAccSetE               = zeros(C.LenGrid,NumjY,NumjY);
    Cutoff                  = 0;
  else
    ...
    for ix = 1:C.LenGrid
      ...
      FirmX   = vec(SimjName(RD.I.iBin == ix & iDropped == 0,:));
      for ijO = 1:NumjY
        Temp                = FirmX == ijO;
    ...
    if isfield(RD.J,'Cutoff') == 1
      Cutoff            = RD.J.Cutoff;
    else
      Cutoff            = linspace(0.5,1,100);
    end
