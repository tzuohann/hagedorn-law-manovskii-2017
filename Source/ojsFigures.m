clear
load Output\ojs\Data.mat
%% RDY_OCorrMZ
HistCell              = 15;
MaxCount              = 0;
FontSize              = 20;
AxisLim               = [0.98 1.001 0 60];
F1 = figure;
set(F1,'Name','RDY_OCorrMZ_OJS')
...
ispec = {'ojs'};
F1 = figure;
set(F1,'Name','AvgProdErrorOJS')
set(F1,'Units','Inches','outerposition',[0 0 11 11]);
set(F1,'PaperOrientation','Landscape');
set(F1,'PaperPositionMode','Auto');
set(F1,'Clipping','off')
[N,X] = hist(ProdError.(ispec{1}).AvgWtDiff_AvgProd,[0.005,0.01,0.015,0.02,0.025,0.03,0.035]);
bar(X,N)
xlabel('Error','fontsize',FontSize)
set(gca,'FontSize',FontSize)
set(gca,'FontSize',FontSize)
...
F1                    = figure;
set(F1,'Name','WorkerFirmSortingAll_OJS')
set(F1,'Units','Inches','outerposition',[0 0 11 11]);
set(F1,'PaperOrientation','Landscape');
set(F1,'PaperPositionMode','Auto');
set(F1,'Clipping','off')
...
FontSize              = 20;
TickMark              = 6;
set(F1,'Name','OutputLossesCombined_OJS')
Group = ProdName < 4;
set(F1,'Units','Inches','outerposition',[0 0 11 11]);
set(F1,'PaperOrientation','Landscape');
set(F1,'PaperPositionMode','Auto');
