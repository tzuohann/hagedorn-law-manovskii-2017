function [I] = simWageStats(Sim,L,SimO)
  ...
  %Replace SimWage with the average wage the worker earns at the firm
  [W,T,FName]      = find(Sim.SimJName);
  Index            = sub2ind(size(Sim.SimWage),W,T);
  Sim.SimWage(Index) = L.iAvWageAtFirm(sub2ind(size(L.iAvWageAtFirm),W,FName));
  
  %Minimum observed wage for each worker.
  I.iWMinWage      = nanmin(Sim.SimWage,[],2);
  
  %Minimum observed wage for each worker.
  I.iWMaxWage      = nanmax(Sim.SimWage,[],2);
