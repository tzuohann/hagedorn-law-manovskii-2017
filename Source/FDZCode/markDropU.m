function [AccSetU,iDropped]     = markDropU(RD,C,SimO,LL,NumjY,jID,iDropped,Sim)
...
%   p                             = getP(C.LenGrid,SimO.Numj,RD.I.iBin,AllW,JobNamejY,zeros(C.NumAgentsSim,1));
  ...
  [ToMoveU, AccSetU, iDropped] = getAccMove(BinoCDF,C,p,Sim,SimO,NumjY,iDropped,RD.I.iBin);
end
...
function [ToMove, AccSet, iDropped] = getAccMove(BinoCDF,C,p,Sim,SimO,NumjY,iDropped,iBin)
  AccSet        = p > 0;
  ToMove        = zeros(size(p));
    ...
    for i1 = 1:NumjY
      if DroppedAnything(i1) == 1
        ...
        if any(OffBin)
          ...
          %Worker who are working at firm 7 and in offbin
          [W,~,~]             = find(Sim.SimJName == i1);
          W                   = unique(W);
          BinW                = iBin(W);
          iDropped(W(BinW == 4))   = 1;
        end
