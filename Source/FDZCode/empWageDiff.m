function jEEWageDiff = empWageDiff(NumjY,jYxMatDen,jYxWageU,jYxAccSetE,L,P,C)
...
%                 jEEWageDiff   = nansum([jEEWageDiff,Tmp.*vec(jYxAccSetE(ix,ijO,:)).*DENEMP],2);
                jEEWageDiff   = nansum([jEEWageDiff,Tmp.*DENEMP],2);
            end
        end
    end
    jEEWageDiff     = jEEWageDiff./full(sum(jYxMatDen(:)));
    %     end
% end
end
